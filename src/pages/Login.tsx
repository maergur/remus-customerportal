          ...
          <CardContent className="pt-6">
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <div className="relative">
                <div className={cn("absolute left-3 top-1/2 -translate-y-1/2 transition-all duration-200 pointer-events-none z-10", (identifierFocused || identifierHasValue) && "opacity-100", !identifierFocused && !identifierHasValue && "opacity-70")}>
                  <div className={cn("transition-all duration-300", inputType === 'email' ? "text-primary" : "text-primary")}>
                    {inputType === 'email' ? <Mail className="h-5 w-5" /> : <Phone className="h-5 w-5" />}
                  </div>
                </div>
                
                <Input id="identifier" type="text" value={identifier} onChange={handleIdentifierChange} onFocus={() => setIdentifierFocused(true)} onBlur={() => setIdentifierFocused(false)} className={cn("h-14 pl-12 pr-12 text-base transition-all duration-200", "border-2 focus:ring-0", identifierFocused ? "border-primary" : "border-input", identifierValid === false && identifier.length > 0 && "border-destructive", identifierValid === true && "border-primary")} placeholder="Telefon veya e-posta" required />
                
                {/* Floating Label */}
                <span className={cn("absolute left-12 transition-all duration-200 pointer-events-none bg-card px-1", identifierFocused || identifierHasValue ? "-top-2.5 text-xs text-primary" : "top-1/2 -translate-y-1/2 text-muted-foreground hidden")}>
                ...
                {identifier.length > 0 && <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    {identifierValid ? <CheckCircle2 className="h-5 w-5 text-primary animate-in zoom-in-50 duration-200" /> : <AlertCircle className="h-5 w-5 text-destructive animate-in zoom-in-50 duration-200" />}
                  </div>}
              </div>
              ...
              <div className="relative">
                ...
                <Input id="password" type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} onFocus={() => setPasswordFocused(true)} onBlur={() => setPasswordFocused(false)} className={cn("h-14 pl-12 pr-12 text-base transition-all duration-200", "border-2 focus:ring-0", passwordFocused ? "border-primary" : "border-input")} placeholder="Şifreniz" required />
                
                {/* Floating Label */}
                <span className={cn("absolute left-12 transition-all duration-200 pointer-events-none bg-card px-1", passwordFocused || passwordHasValue ? "-top-2.5 text-xs text-primary" : "top-1/2 -translate-y-1/2 text-muted-foreground hidden")}>
                  Şifre
                </span>
              ...
              <Button type="submit" className="w-full h-12 text-base font-medium transition-all duration-200" disabled={isLoading || !identifierValid}>
